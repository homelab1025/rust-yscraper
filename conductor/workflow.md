...
8. **Prepare Task Summary:**
   - Draft a detailed summary for the completed task. This should include the task name, a summary of functional changes, and a list of all created/modified files.
   - **CRITICAL:** Focus on *functionality* (what the user can now do) rather than code changes.

9. **Record Task Completion in Plan:**
   - Read `plan.md`, find the line for the completed task, update its status from `[~]` to `[x]`.
   - Append the task summary to the task entry or a designated section in `plan.md` to be included in the phase commit later.
...
2.  **Ensure Test Coverage for Phase Changes:**
    ...
    -   **Step 2.2: Verify and Create Tests:** For each modified code file, verify a corresponding test file exists and validates the phase's functionality.

3.  **Execute Automated Tests with Proactive Debugging:**
    -   Announce and run the automated test suite (e.g., `CI=true npm test`).
    -   If tests fail, propose a fix (max 2 attempts).
...
5.  **Await Explicit User Feedback:**
    -   **PAUSE** and await the user's explicit approval ("yes") before proceeding to commit.
...
7.  **Record Phase Checkpoint SHA:**
    ...
    -   **Step 7.3: Commit Plan Update:** Commit this change with a message like `conductor(plan): Mark phase '<PHASE NAME>' as complete`.
