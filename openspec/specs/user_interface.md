...
The interface uses a sidebar-based layout built with Tailwind CSS:
- **Sidebar**: A left-side navigation panel containing:
    - Logo and Title: "What HN is working on" with a terminal icon.
    - Navigation Links: "LINKS" and "ABOUT" as the primary navigation.
    - Add New Link section: A dedicated section with an "Item ID" input field and an "ADD" button.
...
- Add new Hacker News threads by item ID (via the sidebar).
- View the list of currently tracked threads in the "Task Control Center" table.
...
- A paginated table of comments with columns: Comment, Author, Subcomments (sortable), Date (sortable), Actions.
...
- A footer showing "Showing X of Y comments" with Previous/Next pagination controls.
- A keyboard hint bar showing: `j/k navigate · p pick · d discard`.
